/*
 * esphome-comfoair-card
 * Based on TimWeyand/lovelace-comfoair v0.16.3 (MIT License).
 * Entity defaults adapted for the devsnek/hass-comfoair HA integration.
    ...
    `}};t([mt({attribute:!1})],It.prototype,"hass",void 0),t([ft()],It.prototype,"_config",void 0),t([ft()],It.prototype,"_detectedCount",void 0),It=t([pt($t)],It),console.info("%c ESPHOME-COMFOAIR-CARD %c 0.16.3 ","color:orange;font-weight:bold;background:black","color:white;font-weight:bold;background:dimgray"),window.customCards=window.customCards||[],window.customCards.push({type:bt,name:"ComfoAir Card",preview:!1,description:"Counter-flow heat-exchanger card for the hass-comfoair integration."});const Dt="M6,19 H120 L320,113 H434",Bt="M434,19 H320 L120,113 H6";let Wt=class extends ct{static async getConfigElement(){return document.createElement($t)}static getStubConfig(t){if(!t)return{entity:""};const e=Object.keys(t.states).find(t=>t.startsWith("climate.")&&/comfo|ca\d{3}|wtw/i.test(t));return e?{entity:e,animation:"static",animation_speed_source:"fixed",animation_speed:50,color_scale:"auto",temp_min:-10,temp_max:30,...Rt(t,e)}:{entity:""}}setConfig(t){if(!t)throw new Error(Lt("invalid_config",this.hass?.language));this._config=t}getCardSize(){return 5}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");if(!e||!this._config)return!0;const i=this._config,s=wt.map(t=>i[t]).filter(t=>!!t);return s.some(t=>e.states[t]!==this.hass.states[t])}render(){if(!this.hass||!this._config)return W``;const t=this._config,e=this.hass,i=At(e,t.entity);if(!i)return W`<ha-card>
      ...
      <ha-card class=${o?"animated":""}>
        <div class="hd">
          ...
          <div>
            <div class="ttl">${t.name||"ComfoAir"}</div>
            <div class="st"><span class="dot ${l?"live":""}"></span><span>${n?xt[n]??s:"—"}</span></div>
          </div>
          <div class="grow"></div>
          <div class="recov">
            ${null!=w?W`<b>${w}%</b><span>Heat recovery</span>`:""}
          </div>
        </div>

        <div class="lanes">
          <div class="trow top">
            <div class="tcell l">${C(t.fan_speed_supply,v)}${E(m,t.tempSensor1)}${M("mdi:tree-outline","Outside air")}</div>
            <div></div>
            <div class="tcell r">${k(t.return_air_level)}${E(g,t.tempSensor3)}${M("mdi:home-thermometer-outline","Return air",!0)}</div>
          </div>
          ...
          <div class="trow bot">
            <div class="tcell l">${M("mdi:export","Exhaust air")}${E(f,t.tempSensor2)}${C(t.fan_speed_exhaust,b)}</div>
            <div></div>
            <div class="tcell r">${M("mdi:import","Supply air",!0)}${E(_,t.tempSensor4)}${k(t.supply_air_level)}</div>
          </div>
        </div>
  ...
  if (_ca) {
    ...
    const _STRINGS = {
      ...
      fr: {
        ...
        no_entity:      'Aucune entité climate définie',
        detected:       'entités détectées',
        advanced:       'Avancé / mappage manuel',
        fl_entity:                 'CA350/550 Entité Climate (requise)',
        fl_animation:              'Animation',
        fl_animation_speed_source: 'Source de vitesse',
        fl_animation_speed:        'Vitesse fixe (%)',
        fl_color_scale:            'Échelle de couleurs',
        fl_temp_min:               'Température min (°C)',
        fl_temp_max:               'Température max (°C)',
        fl_show_legend:            "Afficher l'échelle de température",
        fl_name:                   'Nom (optionnel)',
        fl_tempSensor1:            'Température air extérieur',
        fl_tempSensor2:            'Température air extrait',
        fl_tempSensor3:            'Température air repris',
        fl_tempSensor4:            'Température air soufflé',
        fl_filterstatus:           'État du filtre',
        fl_bypass_valve:           'Vanne bypass',
        fl_summer_mode:            'Mode été',
        fl_preheat:                'Registre de préchauffage',
        fl_fan_speed_supply:       'Vitesse ventilateur soufflage',
        fl_fan_speed_exhaust:      'Vitesse ventilateur reprise',
        fl_return_air_level:       'Niveau air repris',
        fl_supply_air_level:       'Niveau air soufflé',
        opt_animated:    'Animé (flux + ventilateurs)',
        opt_static:      'Statique',
        opt_speed_fixed: 'Vitesse fixe (%)',
        opt_speed_level: "Par débit d'air (Soufflage/Reprise %)",
        opt_scale_auto:  'Auto (valeurs actuelles)',
        opt_scale_fixed: 'Fixe (plage manuelle)',
        fan_off: 'Absent', fan_low: 'Niveau 1', fan_medium: 'Niveau 2', fan_high: 'Niveau 3',
        chip_fan: 'Ventil.',    chip_filter: 'Filtre',   chip_bypass: 'Bypass',
        chip_preheat: 'Préchauffe', chip_summer: 'Été',  chip_winter: 'Hiver',
        filter_ok: 'OK',            filter_replace: 'À remplacer',
        bypass_open: 'Ouvert',      bypass_closed: 'Fermé',
        preheat_active: 'Actif',    preheat_off: 'Off',
        lbl_outside: 'Air extérieur', lbl_return: 'Air repris',
        lbl_exhaust: 'Air extrait',   lbl_supply: 'Air soufflé',
        heat_recovery: 'Récupération de chaleur',
        show_history:  "Afficher l'historique",
        ctrl_controls:     'Commandes',
        ctrl_fan_level:    'Vitesse ventilateurs',
        ctrl_temperature:  'Température',
        ctrl_fan_balance:  'Équilibrage',
        ctrl_boost:        'Boost',
        ctrl_reset_errors: 'Réinitialiser erreurs',
        ctrl_reset_filter: 'Réinitialiser filtre',
        bal_balanced: 'Équilibré', bal_supply: 'Soufflage seul', bal_exhaust: 'Reprise seule',
        min_suffix: 'min',
      },
      es: {
        invalid_config: 'Configuración inválida',
        no_entity:      'No hay entidad climate definida',
        detected:       'entidades detectadas',
        advanced:       'Avanzado / mapeo manual',
        fl_entity:                 'CA350/550 Entidad Climate (requerida)',
        fl_animation:              'Animación',
        fl_animation_speed_source: 'Fuente de velocidad',
        fl_animation_speed:        'Velocidad fija (%)',
        fl_color_scale:            'Escala de colores',
        fl_temp_min:               'Temperatura mín (°C)',
        fl_temp_max:               'Temperatura máx (°C)',
        fl_show_legend:            'Mostrar escala de temperatura',
        fl_name:                   'Nombre (opcional)',
        fl_tempSensor1:            'Temperatura aire exterior',
        fl_tempSensor2:            'Temperatura aire expulsado',
        fl_tempSensor3:            'Temperatura aire de retorno',
        fl_tempSensor4:            'Temperatura aire impulsado',
        fl_filterstatus:           'Estado del filtro',
        fl_bypass_valve:           'Válvula bypass',
        fl_summer_mode:            'Modo verano',
        fl_preheat:                'Registro de precalentamiento',
        fl_fan_speed_supply:       'Velocidad ventilador impulsión',
        fl_fan_speed_exhaust:      'Velocidad ventilador retorno',
        fl_return_air_level:       'Nivel aire de retorno',
        fl_supply_air_level:       'Nivel aire de impulsión',
        opt_animated:    'Animado (flujos + ventiladores)',
        opt_static:      'Estático',
        opt_speed_fixed: 'Velocidad fija (%)',
        opt_speed_level: 'Por caudal de aire (Impulsión/Retorno %)',
        opt_scale_auto:  'Auto (valores actuales)',
        opt_scale_fixed: 'Fija (rango manual)',
        fan_off: 'Ausente', fan_low: 'Nivel 1', fan_medium: 'Nivel 2', fan_high: 'Nivel 3',
        chip_fan: 'Ventil.',      chip_filter: 'Filtro',    chip_bypass: 'Bypass',
        chip_preheat: 'Precalent.', chip_summer: 'Verano',  chip_winter: 'Invierno',
        filter_ok: 'OK',            filter_replace: 'Sustituir',
        bypass_open: 'Abierta',     bypass_closed: 'Cerrada',
        preheat_active: 'Activo',   preheat_off: 'Off',
        lbl_outside: 'Aire exterior',   lbl_return: 'Aire de retorno',
        lbl_exhaust: 'Aire expulsado',  lbl_supply: 'Aire impulsado',
        heat_recovery: 'Recuperación de calor',
        show_history:  'Ver historial',
        ctrl_controls:     'Controles',
        ctrl_fan_level:    'Nivel ventiladores',
        ctrl_temperature:  'Temperatura',
        ctrl_fan_balance:  'Equilibrado',
        ctrl_boost:        'Boost',
        ctrl_reset_errors: 'Restablecer errores',
        ctrl_reset_filter: 'Restablecer filtro',
        bal_balanced: 'Equilibrado', bal_supply: 'Solo impulsión', bal_exhaust: 'Solo retorno',
        min_suffix: 'min',
      },
      nb: {
    ...
    _ca.prototype.setConfig = function (cfg) {
      cfg = cfg || {};
      if (!cfg.entity) cfg = Object.assign({}, cfg, { entity: 'climate.comfoair' });
      const _def = {
        animation:              'animated',
        ...
        tempSensor3:  'sensor.comfoair_return_air_temperature',
        tempSensor4:  'sensor.comfoair_supply_air_temperature',
        filterstatus: 'sensor.comfoair_filter_status',
        bypass_valve: 'binary_sensor.comfoair_bypass_valve_open',
        summer_mode:  'binary_sensor.comfoair_summer_mode',
        preheat:      'binary_sensor.comfoair_preheating_state',
        fan_speed_supply:  'sensor.comfoair_supply_fan_speed_rpm',
        fan_speed_exhaust: 'sensor.comfoair_return_fan_speed_rpm',
        return_air_level:  'sensor.comfoair_return_air_level',
        supply_air_level:  'sensor.comfoair_supply_air_level',
      };
      cfg = Object.assign({}, _def, cfg);
      _sc.call(this, cfg);
    };
  }
}